...
class Dms {
    ...
    static parse(dms) {
        ...
        if (!dmsParts.length) return NaN;

        // and convert to decimal degrees...
        let deg = null;
        switch (dmsParts.length) {
            case 3:  // interpret 3-part result as d/m/s
                deg = parseInt(dmsParts[0])/1 + parseInt(dmsParts[1])/60 + parseInt(dmsParts[2])/3600;
                break;
            case 2:  // interpret 2-part result as d/m
                deg = parseInt(dmsParts[0])/1 + parseInt(dmsParts[1])/60;
                break;
